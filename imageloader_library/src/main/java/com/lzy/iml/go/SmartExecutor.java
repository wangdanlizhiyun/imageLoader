...
import com.lzy.iml.go.utils.GoUtil;
...
public class SmartExecutor implements Executor {
    ...
    public List<Runnable> getRunningList() {
    	List<Runnable> list = new ArrayList<Runnable>();
    	for (WrappedRunnable runnable : runningList) {
			list.add(runnable.getRealRunnable());
		}
		return list;
	}

	public List<Runnable> getWaitingList() {
		List<Runnable> list = new ArrayList<Runnable>();
    	for (WrappedRunnable runnable : waitingList) {
			list.add(runnable.getRealRunnable());
		}
		return list;
	}
    ...
    public void cancelCode(int code){
        synchronized (lock) {
            int size = waitingList.size();
            if (size > 0) {
                for (int i = size - 1; i >= 0; i--) {
                    if (waitingList.get(i).getRealRunnable() instanceof LoadTask) {
                        LoadTask loadTask = (LoadTask) waitingList.get(i).getRealRunnable();
                        if (loadTask.mRequest.parentCode == code){
                            waitingList.remove(i);
                        }
                    }
                }
